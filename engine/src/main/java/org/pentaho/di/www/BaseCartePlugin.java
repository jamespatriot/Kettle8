...
public abstract class BaseCartePlugin extends BaseHttpServlet implements CartePluginInterface, CarteRequestHandler {
    ...
    @Override
    public abstract void handleRequest(CarteRequest request) throws IOException;
    ...
    private class CarteRequestImpl implements CarteRequest {
        ...
        @Override
        public CarteResponse respond(int status) {
            if (status >= 400) {
                try {
            ...
            return new CarteResponse() {
                @Override
                public void with(String contentType, WriterResponse response) throws IOException {
                    resp.setContentType(contentType);
                    response.write(resp.getWriter());
                }

                @Override
                public void with(String contentType, OutputStreamResponse response) throws IOException {
                    resp.setContentType(contentType);
